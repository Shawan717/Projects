...
const DetailsSingleCategorie = ({phone}) => {
    ...
    const handleFavourite=()=>{
        ...
        if(!favoritesItem){
            ...
            const isExest=favoritesItem.find(item=>parseInt(item.id)===parseInt(id))
            if(!isExest){
                addToFavorites.push(...favoritesItem,phone)
                localStorage.setItem('favorites',JSON.stringify(addToFavorites))
                swal({
                    title: "Good job!",
                    text: "You clicked the button!",
        ...
        // localStorage.setItem('favorites',JSON.stringify([{name:'sha'}]))
    }  
    return (
...
export default DetailsSingleCategorie;